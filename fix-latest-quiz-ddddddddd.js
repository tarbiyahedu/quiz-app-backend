...
async function fixLatestQuizDDDDDDDDD() {
  try {
    // Connect to MongoDB
    await mongoose.connect(process.env.MONGO_URL);
    console.log('Connected to MongoDB');

    const quizId = '687b6a4fdf7ef98ec717d246'; // DDDDDDDDD quiz
    ...
    console.log(`🔧 FIXING LATEST QUIZ: ${quizId}`);
    ...
    // Add missing answers with appropriate responses
    const answersToAdd = [];
    for (const question of missingQuestions) {
      let answerText = '';
      let isCorrect = false;

      // Generate appropriate answer based on question content
    ...
    console.error('Error fixing latest quiz:', error);
  } finally {
...
if (require.main === module) {
  fixLatestQuizDDDDDDDDD();
}

module.exports = fixLatestQuizDDDDDDDDD; 